...
function ConfigurationPage() {
  ...
  // Compute the backend URL for webhook notifications
  const backendUrl = (() => {
    const saved = localStorage.getItem('apiConfiguration');
    if (saved) {
      try {
        const cfg = JSON.parse(saved);
        if (cfg.webhookForwardUrl) return cfg.webhookForwardUrl;
      } catch (e) { /* ignore */ }
    }
    // On Vercel, use the backend Vercel domain
    if (window.location.hostname.includes('vercel.app')) {
      return 'https://hco-config-payment-page-backend.vercel.app';
    }
    // Local dev: same host but on backend port
    const loc = window.location;
    const port = loc.port === '5173' ? '3001' : loc.port === '5174' ? '3002' : '3001';
  ...
  useEffect(() => {
    ...
    // Load saved JSON payload and migrate if notificationUrl is missing
    const savedJsonPayload = localStorage.getItem('mastercardJsonPayload');
    if (savedJsonPayload) {
      try {
        const parsed = JSON.parse(savedJsonPayload);
        // Migrate: inject or fix notificationUrl in order object
        if (parsed.order && (!parsed.order.notificationUrl || parsed.order.notificationUrl.includes('{orderId}'))) {
          parsed.order.notificationUrl = `${backendUrl}/api/webhook`;
          const migrated = JSON.stringify(parsed, null, 2);
          setJsonPayload(migrated);
          localStorage.setItem('mastercardJsonPayload', migrated);
        } else {
      ...
      <div style={styles.content}>
        ...
        {activeTab === 'api' && (
          <div style={styles.card}>
            ...
            <div style={{ ...styles.formGroup, marginTop: '16px', paddingTop: '24px', borderTop: '1px solid #e2e8f0' }}>
              <label style={styles.label}>Webhook Forwarding URL (Optional)</label>
              <input
                type="url"
                style={styles.input}
                value={apiConfig.webhookForwardUrl}
                onChange={(e) => handleApiConfigChange('webhookForwardUrl', e.target.value)}
                placeholder="https://your-server.com/webhook"
              />
              <p style={{ fontSize: '13px', color: '#718096', marginTop: '6px' }}>
                If set, webhook notifications will also be forwarded to this URL.
              </p>
            </div>
          ...
          <div style={styles.card}>
            ...
            <div style={styles.infoBox}>
              ...
              <p style={{ ...styles.infoText, marginTop: '8px' }}>
                <strong>🔔 order.notificationUrl:</strong> The URL where Mastercard Gateway will send webhook notifications
                when an order is created or updated. This must be an HTTPS endpoint. The server will automatically set this
                to the correct backend webhook URL with the real order ID at checkout time. The webhook notification will be
                displayed on the receipt page after payment.
              </p>
            </div>
